...
# ----------------------------------------------------------------------------------------------------------------------
field_range = range(field_ini,field_fin,field_delta)

for ii_field in field_range:
    # ----------------------------------------------------------------------------------------------------------------------
    ...
    file_norm = data_folder+'/'+error_file
    file_read = open(file_norm,"r")
    error_u   = np.array(file_read.readline().replace('[','').replace(']','').split(','),dtype='float')
    error_v   = np.array(file_read.readline().replace('[','').replace(']','').split(','),dtype='float')
    error_w   = np.array(file_read.readline().replace('[','').replace(']','').split(','),dtype='float')
    file_read.close()
    
    file_error = data_folder+'/'+error_file
    file_error = file_error.replace(".txt","_tot.txt")                     
    file_save  = open(file_error, "a")           
    content    = str(error_u.tolist())+','+str(error_v.tolist())+','+str(error_w.tolist())+'\n'
