...
"""
-------------------------------------------------------------------------------------------------------------------------
main_percolation_shapstruc_uvw_range.py
-------------------------------------------------------------------------------------------------------------------------
...
# Import Packages
# -----------------------------------------------------------------------------------------------------------------------
from py_bin.py_functions.percolation import percolation_uvw,save_percolation_uvw
import os
...
perc_SHAP_file = perc_SHAP_file.replace(".txt","_uvw.txt")
...
for ii in index_range:
    ...
    out_perc                         = percolation_uvw(data_in=data_perc)
    if ii == index_range[0]:
        H_perc     = out_perc["H_perc"]
        nstruc_u   = out_perc["nstruc_u"]
        Vstruc_u   = out_perc["Vstruc_u"]
        nstruc_v   = out_perc["nstruc_v"]
        Vstruc_v   = out_perc["Vstruc_v"]
        nstruc_w   = out_perc["nstruc_w"]
        Vstruc_w   = out_perc["Vstruc_w"]
        nn         = 1
    else:
        nstruc_u   += out_perc["nstruc_u"]
        Vstruc_u   += out_perc["Vstruc_u"]
    ...
    # Average the curves
    # -----------------------------------------------------------------------------------------------------------------------
    nstruc_av_u  = nstruc_u/nn
    nstruc_av_u /= np.max(nstruc_av_u)
    Vstruc_av_u  = Vstruc_u/nn
    nstruc_av_v  = nstruc_v/nn
    nstruc_av_v /= np.max(nstruc_av_v)
    Vstruc_av_v  = Vstruc_v/nn
    nstruc_av_w  = nstruc_w/nn
    nstruc_av_w /= np.max(nstruc_av_w)
    Vstruc_av_w  = Vstruc_w/nn
    save_percolation_uvw(data_in={"nstruc_u":nstruc_av_u,"Vstruc_u":Vstruc_av_u,"nstruc_v":nstruc_av_v,
                                  "Vstruc_v":Vstruc_av_v,"nstruc_w":nstruc_av_w,"Vstruc_w":Vstruc_av_w,
