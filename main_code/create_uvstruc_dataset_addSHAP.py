...
Function to add the shap values to the tangential Reynolds stress structure dataset. In the function the grid-points 
...
data_struc  = {"uvw_folder":uvw_folder,"uvw_file":uvw_file,"Hperc":Hperc,"index":[],"dx":dx,"dy":dx,"dz":dz,
               ...
               "shap_folder":shap_folder,"shap_file":shap_file,"folder":uv_folder,"file":uv_file,
...
for ii in index_range:
    data_struc["index"] = ii
    uv_struc            = uv_structure(data_in=data_struc)
    
    # -------------------------------------------------------------------------------------------------------------------
    # Create the matrix containing the points of the structures
    # -------------------------------------------------------------------------------------------------------------------
    uv_struc.add_SHAP(data_in={"nsamples":nsamples})
