...
#     - hunt_shap_file   : file for saving the coincidence between hunts and shap structures
# -----------------------------------------------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------------------------------------------------
data_hunt   = {"uvw_folder":uvw_folder,"uvw_file":uvw_file,"Hperc":Hperc,"index":0,"dx":dx,
...
for ii in index_range:
    ...
    data_hunt["index"]   = ii
    data_shap["index"]   = ii
    hunt_struc           = hunt_structure(data_in=data_hunt)
    hunt_struc.read_struc()
    shap_struc           = shap_structure(data_in=data_shap)
    shap_struc.read_struc()
    data_out             = calc_coinc(data_in={"data_struc1":shap_struc,"data_struc2":hunt_struc,"save_data":False,
                                               "calc_coin_file":hunt_shap_file,"folder":data_folder,"dy":dy,"dx":dx,
...
save_coinc(data_in={"frac_struc1":frac_struc1,"frac_struc2":frac_struc2,"frac_coinc":frac_coinc,
                    "yplus":yplus,"calc_coin_file":hunt_shap_file,"folder":data_folder})