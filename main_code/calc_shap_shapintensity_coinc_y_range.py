...
"""
-------------------------------------------------------------------------------------------------------------------------
calc_shap_shapintensity_coinc_y.py
-------------------------------------------------------------------------------------------------------------------------
Created on Tue Jun 18 11:30:49 2024

@author: Andres Cremades Botella

Function to calculate the volume occupied by the shap the shap defined with the intensity and the coincidence
as a function of y:
...
from py_bin.py_class.shap_intensity_structure import shap_structure as shap_intensity_structure
import os
...
#     - shap_shapintensity_file : file for saving the coincidence between shap and shap based on intensity structures
# -----------------------------------------------------------------------------------------------------------------------
...
shap_shapintensity_file = folders.shap_shapintensity_file
shap_shapintensity_file = shap_shapintensity_file.replace("shap","shapintensity")
...
# Create the data of the shap structure
# -----------------------------------------------------------------------------------------------------------------------
data_shap_intensity = {"uvw_folder":uvw_folder,"uvw_file":uvw_file,"Hperc":Hperc,"index":index,"dx":dx,
                       "dy":dy,"dz":dz,"L_x":L_x,"L_y":L_y,"L_z":L_z,"rey":rey,"utau":utau,
                       "padding":padding,"data_folder":data_folder,"umean_file":umean_file,
                       "urms_file":urms_file,"sym_quad":True,"filvol":filvol,"shap_folder":shap_folder,
...
for ii in index_range:
    print(ii,flush=True)
    data_shap["index"]           = ii
    data_shap_intensity["index"] = ii
    shap_struc                   = shap_structure(data_in=data_uv)
    shap_struc.read_struc()
    shap_intensity_struc         = shap_intensity_structure(data_in=data_shap)
    shap_intensity_struc.read_struc()
    data_out                     = calc_coinc(data_in={"data_struc1":shap_intensity_struc,"data_struc2":shap_struc,
                                                       "save_data":False,"calc_coin_file":uv_shap_file,
                                                       "folder":data_folder,"dy":dy,"dx":dx,
                                                       "dz":dz,"uvw_folder":uvw_folder,"uvw_file":uvw_file,
...
save_coinc(data_in={"frac_struc1":frac_struc1,"frac_struc2":frac_struc2,"frac_coinc":frac_coinc,
                    "yplus":yplus,"calc_coin_file":shap_shapintensity_file,"folder":data_folder})